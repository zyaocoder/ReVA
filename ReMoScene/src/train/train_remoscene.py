...
train_remoscene.py
=====================
Training script for the ReMoScene module used in ReVA.

Training strategy
-----------------
  Frozen  : RICE vision encoder  +  Qwen2 LLM (all base-model weights)
  Trained : ReMoSceneSelector  ·  MotionBranch  ·  ReMoSceneProjector  ·  gate

Usage example
-------------
torchrun --nproc_per_node=4 train_remoscene.py \\
    --model_id /path/to/base_model \\
    --data_path /path/to/train.jsonl \\
    --image_folder /path/to/videos \\
    --output_dir ./checkpoints/remoscene \\
    --num_train_epochs 3 \\
...
# ---- Path setup -------------------------------------------------------
# Ensure the ReMoScene/ subdirectories are importable regardless of cwd
_THIS_DIR = os.path.dirname(os.path.abspath(__file__))
_PACKAGE_DIR = os.path.abspath(os.path.join(_THIS_DIR, "..", ".."))  # ReMoScene/
_SRC_DIR  = os.path.join(_PACKAGE_DIR, "src")
for _p in (_PACKAGE_DIR, _SRC_DIR):
...
@dataclass
class ReMoSceneArguments:
    """Hyper-parameters for the ReMoScene modules."""

    num_queries        : int   = field(default=256,  metadata={"help": "Number of compressed video tokens (Lq) for ReMoSceneSelector."})
    n_remoscene_layers     : int   = field(default=2,    metadata={"help": "ReMoScene selector depth."})
    remoscene_nhead        : int   = field(default=8,    metadata={"help": "Attention heads (fallback block)."})
    remoscene_expand       : int   = field(default=2,    metadata={"help": "Inner-dim expansion factor."})
    remoscene_d_state      : int   = field(default=16,   metadata={"help": "Mamba SSM state dim."})
    remoscene_d_conv       : int   = field(default=4,    metadata={"help": "Mamba conv kernel size."})
    ...
    rec_chunk_size     : int   = field(default=8,    metadata={"help": "Chunk size used by RecurrentObjectSelector."})
    ...
    use_remoscene_selector    : bool          = field(default=True,  metadata={"help": "Whether to use ReMoSceneSelector (learned compression). False = 3D-pool only."})
...
class ReMoSceneTrainer(Trainer):
    """
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        if os.path.exists(sf_path):
            try:
                ...
                rank0_print(f"[ReMoSceneTrainer] safetensors read failed ({e}), falling back to state_dict()")
                trainable_sd = self.model.trainable_state_dict()
        else:
            trainable_sd = self.model.trainable_state_dict()

        ckpt_path = os.path.join(output_dir, "remoscene_modules.pt")
        torch.save(trainable_sd, ckpt_path)
        rank0_print(f"[ReMoSceneTrainer] Saved trainable modules to {ckpt_path}")
        ...
        if getattr(self.args, "lora_enable", False):
            ...
            rank0_print(f"[ReMoSceneTrainer] Saved LoRA weights to {lora_path}")
...
def train():
    ...
    parser = HfArgumentParser(
        (ModelArguments, DataArguments, TrainingArguments, ReMoSceneArguments)
    )
    model_args, data_args, training_args, remoscene_args = \
    ...
    # ---- Build ReMoScene config -------------------------------------------
    remoscene_cfg = ReMoSceneConfig(
        num_queries        = remoscene_args.num_queries,
        n_remoscene_layers     = remoscene_args.n_remoscene_layers,
        remoscene_nhead        = remoscene_args.remoscene_nhead,
        remoscene_expand       = remoscene_args.remoscene_expand,
        remoscene_d_state      = remoscene_args.remoscene_d_state,
        remoscene_d_conv       = remoscene_args.remoscene_d_conv,
        lq_t               = remoscene_args.lq_t,
        lq_h               = remoscene_args.lq_h,
        lq_w               = remoscene_args.lq_w,
        num_object_queries = remoscene_args.num_object_queries,
        rec_n_layers       = remoscene_args.rec_n_layers,
        rec_nhead          = remoscene_args.rec_nhead,
        rec_chunk_size     = remoscene_args.rec_chunk_size,
        use_remoscene_selector     = remoscene_args.use_remoscene_selector,
        use_motion_branch      = remoscene_args.use_motion_branch,
        use_recurrent_selector = remoscene_args.use_recurrent_selector,
    )

    # ---- Load model ---------------------------------------------------
    rank0_print("Loading frozen base model …")
    model = build_remoscene_model(
        base_model_path = model_args.model_id,
        remoscene_cfg       = remoscene_cfg,
    ...
    if remoscene_args.trainable_ckpt is not None:
        rank0_print(f"Resuming trainable modules from {remoscene_args.trainable_ckpt}")
        sd = torch.load(remoscene_args.trainable_ckpt, map_location="cpu")
    ...
    # ---- Trainer -----------------------------------------------------
    callbacks = []
    live_loss_plot_dir = os.environ.get("REMOSCENE_LIVE_PLOT_DIR")
