# coding=utf-8
"""
BIMBA + Motion Branch wrapper for LLaVA-OneVision-1.5.
...
Architecture (all steps follow the user spec):
  ...
  Step 4  BIMBA Selector: bidirectional selective-scan with interleaved queries
...
Trainable:  BimbaSelector · MotionBranch · BimbaMotionProjector · motion_gate
Frozen   :  everything inside LLaVAOneVision1_5_ForConditionalGeneration
...
class BimbaMotionProjector(nn.Module):
    """
...
class BimbaMotionConfig:
    """
    Hyper-parameters for the BIMBA + Motion extension modules.

    Attributes
    ----------
    num_queries       : int   Total number of compressed video tokens (Lq) for BimbaSelector.
    n_bimba_layers    : int   Depth of the BIMBA selector.
    bimba_nhead       : int   Attention heads (used in fallback attention blocks).
    bimba_expand      : int   Inner-dimension expansion factor.
    bimba_d_state     : int   Mamba SSM state dimension (ignored if no mamba_ssm).
    bimba_d_conv      : int   Mamba local conv kernel size.
    ...
    def __init__(
        self,
        num_queries       : int = 256,
        n_bimba_layers    : int = 2,
        bimba_nhead       : int = 8,
        bimba_expand      : int = 2,
        bimba_d_state     : int = 16,
        bimba_d_conv      : int = 4,
        lq_t              : int = 4,
        lq_h              : int = 8,
        lq_w              : int = 8,
        ...
        rec_n_layers      : int  = 2,
        rec_nhead         : int  = 8,
        use_bimba_selector  : bool = True,
        use_motion_branch   : bool = True,
        use_recurrent_selector: bool = True,
    ):
        self.num_queries        = num_queries
        self.n_bimba_layers     = n_bimba_layers
        self.bimba_nhead        = bimba_nhead
        self.bimba_expand       = bimba_expand
        self.bimba_d_state      = bimba_d_state
        self.bimba_d_conv       = bimba_d_conv
        self.lq_t = lq_t
        self.lq_h = lq_h
        self.lq_w = lq_w
        ...
        self.use_bimba_selector   = use_bimba_selector
        self.use_motion_branch    = use_motion_branch
        self.use_recurrent_selector = use_recurrent_selector


# ============================================================================
# BimbaMotionWrapper  –  the main model class
# ============================================================================

class BimbaMotionWrapper(nn.Module):
    """
    Wraps a frozen LLaVA-OneVision-1.5 model and adds the BIMBA + Motion
    video understanding pipeline.

    Trainable parameters
    --------------------
    - ``bimba_selector``    :  BimbaSelector
    - ``motion_branch``     :  MotionBranch
    - ``projector``         :  BimbaMotionProjector
    ...
    is replaced by the BIMBA + Motion pipeline, and ``inputs_embeds`` /
    ...
    def __init__(
        ...
        bimba_cfg  : Optional[BimbaMotionConfig] = None,
    ):
        super().__init__()
        if bimba_cfg is None:
        ...
        # ---- Trainable modules ----------------------------------------
        self.use_bimba_selector     = bimba_cfg.use_bimba_selector
        self.use_motion_branch      = bimba_cfg.use_motion_branch
        self.use_recurrent_selector = bimba_cfg.use_recurrent_selector

        if self.use_bimba_selector:
            self.bimba_selector = BimbaSelector(
                d_model     = Dv,
                num_queries = bimba_cfg.num_queries,
                n_layers    = bimba_cfg.n_bimba_layers,
                nhead       = bimba_cfg.bimba_nhead,
                expand      = bimba_cfg.bimba_expand,
                d_state     = bimba_cfg.bimba_d_state,
                d_conv      = bimba_cfg.bimba_d_conv,
            )
        ...
        if self.use_recurrent_selector:
            self.recurrent_selector = RecurrentObjectSelector(
                d_model     = Dv,
                num_queries = bimba_cfg.num_object_queries,
                n_layers    = bimba_cfg.rec_n_layers,
                nhead       = bimba_cfg.rec_nhead,
            )
        self.projector = BimbaMotionProjector(d_vision=Dv, d_llm=Dl)
    ...
    def trainable_parameters(self):
        ...
        if self.use_bimba_selector:
            yield from self.bimba_selector.parameters()
        if self.use_motion_branch:
    ...
    def _compress_single_video(
        ...
        """
        Run the full BIMBA + Motion pipeline for one video.
        ...
        # to BimbaSelector so the compressed queries absorb object-level info.
        if self.use_recurrent_selector:
            Z_for_bimba = self.recurrent_selector(Z_3d, pair_feats)  # (T*(HW+N), Dv)
        else:
            Z_for_bimba = Z                                  # (T*HW, Dv)

        # -- Step 4: BIMBA Selector (optional) --------------------------
        if self.use_bimba_selector:
            Q_prime = self.bimba_selector(Z_for_bimba, Q0)  # (Lq, Dv)
        else:
            Q_prime = Q0                                     # (Lq, Dv)
    ...
    def get_video_features_bimba(
        self,
        pixel_values_videos : torch.Tensor,
        video_grid_thw      : torch.LongTensor,
    ) -> List[torch.Tensor]:
        """
        Encode every video in the batch through the BIMBA+Motion pipeline.
    ...
    def _replace_video_tokens(
        ...
        ``inputs_embeds`` with the corresponding compressed BIMBA embeddings,
    ...
    def forward(
        ...
        **kwargs,
        ...
        Video inputs are processed through the BIMBA + Motion pipeline.
        ...
        if inputs_embeds is None:
            ...
            # ---- Videos: BIMBA + Motion pipeline ----
            if pixel_values_videos is not None:
                all_video_embeds = self.get_video_features_bimba(
    ...
    def prepare_generation_inputs(
        ...
        generation.  The BIMBA+Motion pipeline runs once here; subsequent
    ...
    def generate(
        ...
        **gen_kwargs,
        ...
        1. Runs the BIMBA+Motion pipeline to build ``inputs_embeds``.
...
def build_bimba_motion_model(
    ...
    wrapper = BimbaMotionWrapper(base_model, bimba_cfg=bimba_cfg)
