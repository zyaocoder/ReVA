"""
Smoke test for the Motion-OV1.5 pipeline
(BimbaSelector + MotionBranch + RecurrentObjectSelector).

Tests all new/modified components with dummy tensors (no base model required).
Run from the ds/ directory:
    python test_motion_ov15_smoke.py
"""
...
# without needing the full LLaVA-OneVision-1.5 + transformers installation.
# ---------------------------------------------------------------------------
sys.path.insert(0, "/home/x-zyao1/LIKAI/LLM-Baseline/LLaVA-OneVision-1.5/ds")
...
from llavaonevision1_5.bimba_motion import (  # noqa: E402
    BimbaMotionConfig,
    BimbaSelector,
    MotionBranch,
    RecurrentObjectSelector,
    BimbaMotionProjector,
    pool3d_init_queries,
    resize_queries,
)
...
Lq  = 32    # BimbaSelector queries (real: 256)
...
def test_compress_pipeline():
    ...
    bimba_sel = BimbaSelector(d_model=Dv, num_queries=Lq,
                               n_layers=1, nhead=4, expand=2,
                               d_state=8, d_conv=4, prefer_mamba=False)
    motion_br = MotionBranch(d_model=Dv)
    rec_sel   = RecurrentObjectSelector(d_model=Dv, num_queries=N,
                                        n_layers=1, chunk_size=CS, nhead=4)
    projector = BimbaMotionProjector(d_vision=Dv, d_llm=Dl)
